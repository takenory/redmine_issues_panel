class IssueCard < Issue
  attribute :reorderd, :boolean, default: false
  validate do
  ...
  def move!(attributes={})
    ...
    self.transaction do
      ...
      if attributes[:orderd_issue_ids].is_a?(Array) && attributes[:orderd_issue_ids].any?
        IssueCardPosition.update_positions!(attributes[:orderd_issue_ids])
        self.reorderd = true
      end
    end
  end
